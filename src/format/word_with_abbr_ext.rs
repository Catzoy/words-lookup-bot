...
pub fn compose_word_with_abbrs_determined<V, E, Formatter: LookupFormatter<Result<V, E>>>(
    ...
    word: &str,
    results: &(
        anyhow::Result<Vec<WordDefinition>>,
        anyhow::Result<Vec<AbbreviationDefinition>>,
    ),
    on_empty: fn() -> V,
) -> Result<V, E> {
    match results {
        (Ok(words), Ok(abbrs)) => match (words.len(), abbrs.len()) {
            (0, 0) => Ok(on_empty()),
        ...
        (Ok(words), _) => compose_word_defs(formatter, word, words),

        (_, Ok(abbrs)) => compose_abbr_defs(formatter, word, abbrs),

        (Err(_), Err(_)) => Ok(on_empty()),
    }
}
